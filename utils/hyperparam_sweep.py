""" Launch multiple jobs on a server with multiple GPUs.
    ...
    1) the name, 2) the types of model settings to search (which each have
    specific hyperparams), 3) the hparams that will stay constant, 4) the
    hparams and values to search, 5) whether to take the 'cross_product' of all
    searched hparams values, or iterate through the lists of values for each
    hparam 'in_order'.
    ...
    An example job file format is as follows:
        ...
        --encoder_hidden_size=[400,600,800] --decoder_hidden_size=[400,600,800]kk
        in_order
    ...
    Usage: python hparam_tune.py --available_gpus 1 2 3 4 5 6 7 \
    					--specification=./jobs_to_run/hparam_tune1.txt
"""
...
class JobGroup:
    def __init__(self, name, model_types, stable_hparams, search_string,
        ...
        self.stable_hparams += '--extra_save_dir=' + TUNE_DIRECTORY
